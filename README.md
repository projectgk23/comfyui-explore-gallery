...
2. 画像を**クリックで選択**（緑枠が付く）。`全選択 / 全解除` も可。サムネ右下の**拡大アイコン**で拡大表示、**📋 アイコン**でワークフローをコピー。
...
8. `T2I_HQ_B_brushup` を Queue（incremental_image が1枚ずつ消化）。
...
- **クリックで選択**: 画像クリックで選択（緑枠）。拡大はサムネ右下の**拡大アイコン**から
...
- **拡大表示**: 全解像度プレビュー。**ホイールでズーム / ドラッグでパン**（クリックでも2倍ズーム・トグル）。背景 / `✕` / `Esc` で閉じる
  - キー操作: `←` / `→` 送り、`↑` 選択トグル、`↓` ゴミ箱（`_trash` では完全削除）、`i` 情報パネル、`c` ワークフローコピー、`Esc` 閉じる
